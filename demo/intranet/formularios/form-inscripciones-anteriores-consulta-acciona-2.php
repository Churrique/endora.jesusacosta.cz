...
<table width="80%" border="0" cellspacing="0" cellpadding="0" class="tabla_body">
  ...
  if ( isset($_POST['btnVerTodo']) ) {
	  header('Location: form-inscripciones-anteriores-ver-todo-2.php?ORDENACION='.$_POST['rbtOrden']);
	  exit;
  }
  ?>
  <form name="FrmResuDCon" method="post" action="form-inscripciones-anteriores-consulta-2.php">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td height="30" colspan="5" class="gradiente_gris2"><span class="texto_decorado_blanco_amarillo">Consulta de Insrcipciones Anteriores (del SiDICEs). Criterio de Búsqueda: &laquo;
        <?php
      ...
      <tr>
        ...
        <?php
		  if ( isset($_POST['btnBuscar']) ) {
			  if ( isset($_POST['txtBci']) && $_POST['txtBci'] != '' ) {
				  //buscar por cédula
				  $resultado_de_busqueda = CDe1InsAnt('cedula',$_POST['txtBci']);
				  echo $resultado_de_busqueda;
			  }
			  if ( isset($_POST['txtNomB']) && $_POST['txtNomB'] != '' ) {
				  //buscar por nombre
				  $resultado_de_busqueda = CDe1InsAnt('nombre',$_POST['txtNomB']);
				  echo $resultado_de_busqueda;
			  }
			  if ( isset($_POST['txtApeB']) && $_POST['txtApeB'] != '' ) {
				  //buscar por apellido
				  $resultado_de_busqueda = CDe1InsAnt('apellido',$_POST['txtApeB']);
				  echo $resultado_de_busqueda;
			  }
		  }
		?>
