<?php
//************************
//Version 7.x 22-11-2020 *
//************************
//define('OLD_ERROR_HANDLER',set_error_handler("ErrPer"));
//error_reporting(E_ALL ^ E_NOTICE);
//* FUNCIONES DE VALIDACIONES DE FECHA
//funci�n para contrastar la tabla de registro_estudiante con el registro de inscritos en el sitio www.inscripciones.uc.edu.ve
...
function conectar($host,$usuario,$clave,$database)
{
    ...
    $conn = mysqli_connect($host, $usuario, $clave, $database);
   if (!$conn) {
    echo "Error: No se pudo conectar a MySQL." . '<br>';
    echo "errno de depuraci�n: " . mysqli_connect_errno() . '<br>';
    echo "error de depuraci�n: " . mysqli_connect_error() . '<br>';
    exit;
   }
...
{
    //$resultado=mysql_query($query,$conexion) or die('Consulta fallida. ' . mysql_error() . '<br />[funci�n ejecutar]:<br />' . $query);
    $resultado=mysqli_query($conexion, $query) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n ejecutar]:<br />' . $query);
    return $resultado;
}
...
function validar_cedula($cedula)
{
    ...
    {
        $a[0]=false;
        $a[1]="Por favor introduzca una c�dula";
    }
    else
    {
        ...
        {
            $a[0]=false;
            $a[1]="La c�dula debe ser un valor num�rico";
        }
        else
        {
            ...
            {
                $a[0]=false;
                $a[1]="La c�dula introducida es incorrecta";
            }
        }
    }
    return $a;
...
function validar_deposito($deposito)
{
    ...
    {
        $a[0]=false;
        $a[1]="Por favor introduzca el n�mero de dep�sito";
    }
    else
    {
        ...
        {
            $a[0]=false;
            $a[1]="El dep�sito debe contener solo n�meros";
        }
    }
    return $a;
}
...
function validar_clave($clave,$cedula)
{
    ...
    {
        $a[0]=false;
        $a[1]="Error: La clave solo debe contener n�meros y letras";
    }
    else
    {
        if($clave==$cedula)
        {
            $a[0]=false;
            $a[1]="Error: La clave debe ser distinta de su c�dula";
        }
        else
        {
            if(strlen($clave)<6)
...
{
	$consulta=mysqli_query($conn, "SELECT * FROM lista_paises order by pais");
...
function obtener_nombre_modalidad($conexion,$codigo_modalidad)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_modalidad]:<br />' . $query);
...
function obtener_nombre_escuela($conexion,$codigo_escuela)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_escuela]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
...
function obtener_nombre_facultad($conexion,$codigo_facultad)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_facultad]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
...
function obtener_nombre_inscripcion($conexion,$codigo_tipo_inscripcion)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_inscripcion]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
...
function lista_facultades($conn,$codigo_facultad)
{
    ...
    {
        $consulta=mysqli_query($conn, "SELECT * FROM facultades where codigo_facultad='$codigo_facultad'");
        echo "<select name='listado_facultades'>";        
    }
    else
    {
        $consulta=mysqli_query($conn, "SELECT * FROM facultades where codigo_facultad NOT IN ('800','900')order by nombre_facultad");
...
function lista_carreras($conn,$codigo_facultad)  
{
    ...
    if($codigo_facultad !="800")
    {
        $consulta=mysqli_query($conn, "SELECT * FROM carreras where codigo_facultad='$codigo_facultad'");
    }
    else
    {
        $consulta=mysqli_query($conn, "SELECT * FROM carreras where codigo_facultad NOT IN ('800','900')order by nombre_escuela");
    }
...
function lista_carreras2($conn,$codigo_facultad,$codigo_escuela) 
{
    ...
    {
        $consulta=mysqli_query($conn, "SELECT * FROM carreras order by nombre_escuela");
    }
...
{
    $consulta=mysqli_query($conn, "SELECT * FROM modalidades order by nombre_modalidad");
    // Voy imprimiendo el primer select compuesto por los paises
...
{
    $consulta=mysqli_query($conn, "SELECT * FROM tipo_retiros order by codigo_tipo_retiro");
...
function cuadro_historial($conexion,$tabla_historial_inscripciones,$cedula)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_historial]:<br />' . $query);
...
function cuadro_usuarios($conexion,$tabla_usuarios)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_usuarios]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
        if($aux['activo']==1)
        {
...
function cuadro_autoridad($conexion,$tabla_autoridad_carnet)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_autoridad]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
...
function cuadro_dependencias($conexion,$tabla_dependencias_carnet)
{
    ...
    //$result=mysql_query($sql,$conexion) or die('Consulta fallida. ' . mysql_error() . '<br />[funci�n cuadro_dependencias]:<br />' . $query);
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_dependencias]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
...
function cuadro_estudiante($conexion,$tabla_cargoestudiante_carnet)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion). '<br />[funci�n cuadro_estudiante]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
...
function cuadro_prestamos($conexion,$cedula)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_prestamos]:<br />' . $query);
    if(mysqli_num_rows($result)==1)
    {
        ...
        echo "<tr><td colspan=\"2\" class=\"Estilo18\"><br/>Fecha del Pr�stamo: ".$fecha_prestamo."</td></tr>";
        echo"<tr><td colspan=\"2\" class=\"Estilo11\"><br/><br/><div align=\"center\">Si la persona ya devolvi� los documentos y est� solvente presione&nbsp;&nbsp;<input type=\"submit\" name=\"devolver\" value=\"AQUI\" class=\"boton1\" /></td></tr>";
    }
    else
    {
...
{
    $sql="SELECT * FROM facultades WHERE codigo_facultad= '$codigo_facultad'";
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_regimen_facultad]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
    return $aux['regimen'];
}
//funcion que me retorna el monto de un arancel espec�fico
function obtener_monto_arancel($conexion,$codigo_arancel)
{
    $sql="SELECT * FROM aranceles WHERE codigo_arancel= '$codigo_arancel'";
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_monto_arancel]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
    return $aux['monto'];
}
function lista_operaciones_usuario($conn,$usuario)
{
    $consulta=mysqli_query($conn, "SELECT permisos.login,permisos.codigo_operacion,operaciones.nombre_operacion from permisos join operaciones on permisos.codigo_operacion=operaciones.codigo_operacion where permisos.login='$usuario' order by operaciones.nombre_operacion");
...
function obtener_link_operacion($conexion,$codigo_operacion)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_link_operacion]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
    return $aux['link'];
}
...
function usuario_autorizado($conexion,$usuario,$codigo_operacion)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n usuario_autorizado]:<br />' . $query);
    if(mysqli_num_rows($result)==1)
    {
        return 1;  //verdadero
    }
...
function cuadro_usuarios_permisos($conexion,$tabla_usuarios)
{
    $i=0;
    $sql="select * from $tabla_usuarios order by login";
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_usuarios_permisos]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
        if($aux['activo']==1)
        {
...
//listar permisos usuario: muestra la permisolog�a asociada a un usuario del sistema
function listar_permisos_usuario($conexion,$usuario)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n listar_permisos_usuario]:<br />' . $query);
    if(mysqli_num_rows($result)==0)
    {
        echo "<div align=\"center\" class=\"Estilo14\">A este usuario a�n no se le ha asignado alguna permisolog�a</div>";
    }
    else
    {
...
{
    $consulta=mysqli_query($conn, "SELECT * from operaciones order by nombre_operacion");
    // Voy imprimiendo el primer select compuesto por las operaciones de usuario
...
function cuadro_resumen_inscritos($conexion,$tabla_inscritos,$facultad,$escuela,$fecha_inicio,$fecha_final)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_resumen_inscritos]:<br />' . $query);
    $cerrar=false;
...
function cuadro_resumen_preinscritos($conexion,$tabla_inscritos,$facultad,$escuela,$fecha_inicio,$fecha_final)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_resumen_preinscritos]:<br />' . $query);
    $cerrar=false;
    $facultad_actual="999"; //inicializo valores
    $escuela_actual="999"; //inicializo valores
    while($aux=mysqli_fetch_array($result))
...
function cuadro_historial_carnet($conexion,$tabla_historial_estudiante,$cedula)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_historial_carnet]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
        $carrera=obtener_nombre_escuela($conexion,$aux['carrera']);
        ...
        $impreso= $aux['impreso_por'];
		$motivo=obtener_nombre_motivo($conexion, $aux['codigo_motivo']);
        echo "<tr class=\"Estilo8\"><td align=\"center\">".$fecha_carnetizacion."</td><td align=\"center\">".$facultad."</td><td align=\"center\">".$carrera."</td><td align=\"center\">".$fecha_vencimiento."</td><td align=\"center\">".$impreso."</td><td align=\"center\">".$motivo."</td></tr>";
    }
}
//cuadro historial: muestra un cuadro con el historial de carnetizacion de un estudiante
function cantidad_carnet_motivos($conexion,$tabla_historial_estudiante,$fechad,$fechah)
{
    $i=0;
    $sql="SELECT h.codigo_motivo codmot, m.nombre_motivo nom, COUNT(h.cedula) as cantidad
			  from $tabla_historial_estudiante h, motivo_tramite m 
			  where h.codigo_motivo = m.codigo_motivo
			    and (DATE_format(h.fecha_carnetizacion,'%d/%m/%Y') between '$fechad' and '$fechah')				
			  GROUP BY h.codigo_motivo, m.nombre_motivo";
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_historial_carnet]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
        $codigomotivo=$aux['codmot'];
        $nombre=$aux['nom'];
        $cantidad=$aux['cantidad'];
        echo "<tr class=\"Estilo8\"><td align=\"center\">".$codigomotivo."</td><td align=\"center\">".$nombre."</td><td align=\"center\">".$cantidad."</td></tr>";
    }
}
...
function cuadro_historial_carnet_impresion($conexion,$tabla_historial_estudiante,$cedula)
{
    $i=0;
    $sql="select * from $tabla_historial_estudiante where cedula=$cedula order by fecha_carnetizacion";
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_historial_carnet_impresion]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
        $carrera=obtener_nombre_escuela($conexion,$aux['carrera']);
        $facultad=obtener_nombre_facultad($conexion,$aux['facultad']);
...
function cuadro_historial_carnet_personal($conexion,$tabla_historial_personal,$cedula)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n cuadro_historial_carnet_personal]:<br />' . $query);
    while($aux=mysqli_fetch_array($result))
    {
...
function obtener_nombre_dependencia($conexion,$codigo_dependencia)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_dependencia]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
...
function obtener_nombre_tipo($conexion,$codigo_tipo)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_tipo]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
...
{
    $consulta=mysqli_query($conn, "SELECT * FROM tipo_personal order by nombre_tipo");
...
{
    $consulta=mysqli_query($conn, "SELECT * FROM dependencias order by nombre_dependencia");
...
{
    $consulta=mysqli_query($conn, "SELECT * FROM cargo_estudiante order by nombre_cargo");
...
function obtener_nombre_cargo($conexion,$codigo_cargo)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_cargo]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
    return $aux['nombre_cargo'];
}
//CARNET: funcion para generar listado de motivos de tramitar un carnet en la Universidad
function lista_motivos($conn)
{
    $consulta=mysqli_query($conn, "SELECT * FROM motivo_tramite order by codigo_motivo");
    // Voy imprimiendo el primer select compuesto por los motivos
    echo "<select name='listado_motivos'>";
    echo "<option value=\"\"></option>";
    while($registro=mysqli_fetch_row($consulta))
    {
        if(isset($_POST['listado_motivos']) && $_POST['listado_motivos']==$registro[0])
        {
            echo "<option value='".$registro[0]."' selected='selected'>".$registro[1]."</option>";
        }
        else
...
//CARNET: funcion que me retorna el nombre de un motivo de tramite dado el codigo
function obtener_nombre_motivo($conexion,$codigo_motivo)
{
    $sql="SELECT * FROM motivo_tramite WHERE codigo_motivo= '$codigo_motivo'";
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci?n obtener_nombre_motivo]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
    return $aux['nombre_motivo'];
}
//CARNET: funcion para generar listado de Autoridades
function lista_autoridad($conn)
{
    $consulta=mysqli_query($conn, "SELECT * FROM autoridad order by nombre_autoridad");
...
function obtener_nombre_autoridad($conexion,$codigo_autoridad)
{
    ...
    $result=mysqli_query($conexion, $sql) or die('Consulta fallida. ' . mysqli_error($conexion) . '<br />[funci�n obtener_nombre_autoridad]:<br />' . $query);
    $aux=mysqli_fetch_array($result);
