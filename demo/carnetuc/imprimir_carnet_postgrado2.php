        ...
        else
        {
           ...
           else
           {
                ...
                else
                {
                    ...
                    $sql = "INSERT INTO $tabla_historial_postgrado(`cedula`,`fecha_carnetizacion`,`fecha_vencimiento`,`carrera`,`facultad`,`impreso_por`) VALUES ('$_SESSION[cedula]','$hoy','$_SESSION[fecha_vencimiento]','$_SESSION[codigo_carrera]','$_SESSION[facultad]','$_SESSION[usuario]')";
                    $result = ejecutar($sql,$conexion);
                    $sql="select * from $tabla_estudiante_postgrado where cedula=$_SESSION[cedula]";   
                    $result = ejecutar($sql,$conexion);  
                    if (mysql_num_rows($result)>=1)
                    {
                      $sql="update $tabla_estudiante_postgrado set `generado`='1' where cedula=$_SESSION[cedula]";
                      $result = ejecutar($sql,$conexion);  
                    } 
                    ...
                    {
                       echo "<script>javascript:openwindow('imprimir_carnet_postgrado3.php',800,400)</script>";       
                    }
                    else
                    {
                      echo "<script>javascript:openwindow('imprimir_carnet_postgrado4.php',800,400)</script>";     
                    }       
                } 
            }    
          }
...
include("formas/imprimir_carnet_postgrado2.html");
