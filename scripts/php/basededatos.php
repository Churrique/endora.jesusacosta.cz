...
function conecta($p_ser = SERVIDOR, $p_user = USUARIO, $p_pass = CONTRASENIA, $p_bdd = BASEDEDATOS, $p_puer = PUERTO) {
    ...
    if (!$enlace) {
      header("Location: https://www.jesusacosta.cz/stranky/error.php?cislo=".mysqli_connect_errno()."&popis=".mysqli_connect_error()."<br>No se pudo conectar...!");
      exit;
    }
    if (is_object($enlace)) {
        return($enlace);
        ...
        header("Location: https://www.jesusacosta.cz/stranky/error.php?popis= Cuidado! Error No Depurado...");
        exit;
      }
}
...
function ArmSelect( $p_la_tabla = null, $p_el_campo = null, $p_valor_name = null, $p_valor_id = null, $p_valor_onchange = null, $p_valor_onclick = null ) {
   // mysqli_fetch_assoc($el_resultado)
   // es igual a
   // mysql_fetch_array($el_resultado, MYSQL_ASSOC)
   // Este crea asociativo y por número
